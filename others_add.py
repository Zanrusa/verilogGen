...
DICT_TYPE = {0:'r',
             1:'w'}
class Ui_othersAdd(QtWidgets.QDialog):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
    ...
    def setupUi(self, Form):
        ...
        self.basicComplist = []
##        self.basicComplist.append(Comp_stypeMember('others',Form))
##        self.basicComplist[0].setGeometry(QtCore.QRect(10, 50, 700, 80))
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "Other Type Define"))
    ...
    def ifPlus(self):
        _temp =Comp_stypeMember('others',self)
    ...
    def ifAddgroup(self):
        ...
        for comp in self.basicComplist:
            ...
            if _index == 0:
                self.reglist[_iname] = moduleType.judgeType(iname=_iname,itype=_itype,iwidth=_iwidth)
            elif _index == 1:
                self.wirelist[_iname] = moduleType.judgeType(iname=_iname,itype=_itype,iwidth=_iwidth)
            else:
                pass

    def loadElement(self,others):
        for num in range(len(others)):
            for name in others[num]:
                _temp = Comp_stypeMember('others',self)
                _temp.nameEdit.setText(others[num][name].getName)
                _temp.typeInput.setCurrentIndex(num)
                _temp.widthEdit.setText(str(others[num][name].getWidth))
                _temp.setGeometry(QtCore.QRect(10, 50+len(self.basicComplist)*70, 700, 80))
                self.basicComplist.append(_temp)
